...
public class Evolver {
		...
		while (MV.currentRound < MV.MIN_ROUNDS || evolutionIsImproving()) {
			...
			for (int k = 0; k < MV.SAMPLE_SIZE; k++) {
				...
				for (int i = 0; i < triggeredCount; i++) {
					...
					currentModules[i].fitness = -1 * Math.abs(output - MV.currentOutput);
					if (currentModules[i].fitness != currentModules[i].fitness
						...
						currentModules[i].fitness = -1 * Double.MAX_VALUE;
	...
	private void test() {
		...
		try {
			...
			int testCases = 100;
			double[][] evaluationCases = new double[testCases][numOfInputs + 1];
			in = new BufferedReader(new FileReader(
					"C:\\Users\\taor9299\\Box Sync\\Java workspace\\ForexTrader\\evaluationCases.txt"));
			for (int i = 0; i < testCases; i++) {
				line = in.readLine();
				tokens = new StringTokenizer(line);
				for (int j = 0; j < numOfInputs + 1; j++) {
					evaluationCases[i][j] = Double.parseDouble(tokens.nextToken().trim());
				}
			}
			MV.fitnessCases = evaluationCases;
			in.close();
			String outputString;
			fileWriter = new WriteFile("output.txt", false);
			for (int i = 0; i < testCases; i++) {
				MV.retreiveInput(i);
				double avgPredict = 0, output;
				double[] x;
				...
				System.out.println(x[numberOfPredict/2] + " " + numberOfPredict);
				outputString = Double.toString(x[numberOfPredict/2]) + "";
	...
	private void evolveModules() {
		...
		for (int i = 0; i < MV.EVO_ROUNDS_PER_CASE; i++) {
			...
			if (random < MV.CLIQUE_BORROW_PROB) {
				...
				newModule.fitness = -1 * Math.abs(output - MV.currentOutput);
