...
public class tiny_gp {
  ...
  void setup_fitness(String fname) {
    try {
      ...
      new BufferedReader(
      		    new
      		    FileReader("C:\\Users\\taor9299\\Box Sync\\Java workspace\\ForexTrader\\sin-data - Copy.txt"));
      line = in.readLine();
  ...
  char[] subtreeMutation(char[] parent) {
		...
		subtreeLen = grow(buffer, 0, MAX_LEN, DEPTH);
		
		  while (subtreeLen < 0 )
			  subtreeLen = grow( buffer, 0, MAX_LEN, DEPTH );

		  newSubtree = new char[subtreeLen];

		    System.arraycopy(buffer, 0, newSubtree, 0, subtreeLen ); 
  ...
  void evolve() {
    ...
    for ( gen = 1; gen < GENERATIONS; gen ++ ) {
      ...
      else if (random < CROSSOVER_PROB + P_MUT_PROB) {
